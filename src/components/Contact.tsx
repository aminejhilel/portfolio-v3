...
import {
  ...
  ArrowRight, Zap, Clock, CheckCircle, Star
} from "lucide-react";
...
export default function Contact() {
  ...
  return (
    <section id="contact" className="py-28 paper-canvas relative border-t-2 border-slate-900 overflow-hidden text-slate-900 section-accent-top">
      {/* Dot background */}
      ...
      {/* Ambient blobs */}
      <div className="absolute top-0 left-1/4 w-80 h-80 rounded-full bg-amber-200/25 blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 right-1/4 w-96 h-96 rounded-full bg-emerald-200/20 blur-3xl pointer-events-none" />
      ...
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <h2 className="section-heading text-6xl sm:text-7xl">
            CONTACT &{" "}
            <span className="relative inline-block">
              <span className="text-amber-500">INFOS</span>
              <svg className="absolute -bottom-1 left-0 w-full" height="10" viewBox="0 0 200 10" preserveAspectRatio="none" fill="none">
                <motion.path
                  d="M2 7 Q50 3 100 6 Q150 9 198 4"
                  stroke="#f59e0b"
                  strokeWidth="3"
                  strokeLinecap="round"
          ...
          {/* Availability Badge */}
          <div className="inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full bg-emerald-100 border-2 border-emerald-400 text-emerald-800 text-sm font-mono font-bold shadow-[3px_3px_0px_rgba(16,185,129,0.3)]">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="lg:col-span-5 space-y-6">
            ...
            <div className="bg-white rounded-3xl p-7 border-2 border-slate-900 shadow-[6px_6px_0px_rgba(15,23,42,1)] space-y-5 relative overflow-hidden">
              {/* Top gradient accent */}
              ...
              <h3 className="font-heading text-lg font-extrabold text-slate-900 border-b-2 border-dashed border-slate-200 pb-3 pt-3 flex items-center gap-2">
              ...
              <div className="space-y-3">
                {contactItems.map((item, idx) => {
                  const inner = (
                    <div className={`flex items-center gap-4 p-4 rounded-2xl ${item.cardBg} border-2 ${item.cardBorder} transition-all group cursor-pointer`}>
                      <div className={`p-2.5 rounded-xl ${item.bg} text-slate-950 border-2 border-slate-900 shadow-[2px_2px_0px_rgba(15,23,42,0.3)] flex-shrink-0`}>
                        {item.icon}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest">{item.label}</div>
                        <div className={`text-sm font-bold text-slate-900 ${item.textHover} transition-colors truncate`}>{item.value}</div>
                      </div>
                      {item.href && <ArrowRight className="w-4 h-4 text-slate-300 group-hover:text-slate-700 flex-shrink-0 transition-transform group-hover:translate-x-0.5" />}
                    </div>
                  );
                  return item.href ? (
              ...
              <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200">
                ...
                <span className="text-xs font-mono font-bold text-slate-500">Réponse habituelle sous <span className="text-amber-600">24h</span></span>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-1">
                ...
                <Link
                  href="/cv"
                  className="flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-slate-900 text-white font-bold text-sm border-2 border-slate-900 shadow-[3px_3px_0px_rgba(245,158,11,0.5)] hover:shadow-none hover:translate-x-[3px] hover:translate-y-[3px] transition-all duration-150"
                >
            ...
            {/* Languages Card */}
            <div className="bg-white rounded-3xl p-6 border-2 border-slate-900 shadow-[6px_6px_0px_rgba(15,23,42,1)] space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                {languages.map((lang, idx) => (
                  <motion.div
                    ...
                    className={`p-3 rounded-2xl bg-gradient-to-b ${lang.color} border-2 ${lang.border} text-center space-y-1.5 cursor-default shadow-sm hover:shadow-md transition-shadow`}
                  >
                    ...
                    <div className="inline-flex items-center gap-1">
                      ...
                      <span className="text-[10px] font-mono font-bold text-slate-500">{lang.level}</span>
                    </div>
                  </motion.div>
                ))}
              </div>
          ...
          <motion.div variants={itemVariants} className="lg:col-span-7">
            <div className="bg-white rounded-3xl p-7 sm:p-10 border-2 border-slate-900 shadow-[8px_8px_0px_rgba(15,23,42,1)] relative overflow-hidden">
              {/* Top accent */}
              <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-emerald-400 via-sky-400 to-blue-400 rounded-t-3xl" />
              {/* Tape */}
              <div className="absolute -top-3 right-12 w-32 h-5 bg-emerald-300 border-x border-emerald-400 -rotate-1 shadow-sm opacity-90" />

              <h3 className="font-heading text-xl font-extrabold text-slate-900 mb-6 flex items-center gap-2.5 pt-1">
                <div className="w-8 h-8 rounded-xl bg-emerald-400 border-2 border-slate-900 flex items-center justify-center shadow-[2px_2px_0px_rgba(15,23,42,0.3)]">
              ...
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={field.id} className="space-y-1.5">
                      <label htmlFor={field.id} className="text-xs font-bold text-slate-600 flex items-center gap-1">
                        {field.label}
                        <span className="text-amber-500">*</span>
                      </label>
                      <div className={`relative rounded-xl border-2 transition-all duration-200 ${
                        focused === field.id
                          ? "border-slate-900 shadow-[0_0_0_3px_rgba(245,158,11,0.2)]"
                ...
                <div className="space-y-1.5">
                  <label htmlFor="subject" className="text-xs font-bold text-slate-600 flex items-center gap-1">
                    Sujet
                    <span className="text-amber-500">*</span>
                  </label>
                  <div className={`relative rounded-xl border-2 transition-all duration-200 ${
                ...
                <div className="space-y-1.5">
                  <label htmlFor="message" className="text-xs font-bold text-slate-600 flex items-center gap-1">
                    Message
                    <span className="text-amber-500">*</span>
                  </label>
                  <div className={`relative rounded-xl border-2 transition-all duration-200 ${
                ...
                {/* Tips */}
                <div className="flex items-start gap-2.5 px-4 py-3 rounded-xl bg-amber-50 border-2 border-amber-200">
